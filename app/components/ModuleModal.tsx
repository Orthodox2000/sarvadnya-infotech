  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
      <div className="bg-white rounded-3xl p-6 md:p-10 max-w-3xl w-full shadow-2xl relative animate-in zoom-in-95 duration-200 max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
        ...
        <div className="mb-8">
          <span className="inline-block px-3 py-1 bg-indigo-50 text-indigo-600 text-[10px] font-black uppercase tracking-widest rounded-full border border-indigo-100 mb-4">
            {module.category}
          </span>
          <h3 className="text-2xl md:text-4xl font-black mb-4 text-[#0f0529]">{module.title}</h3>
          <p className="text-slate-600 leading-relaxed text-sm md:text-base border-l-4 border-[#7338a0] pl-6 italic">
          ...
          <div>
            ...
            <ul className="space-y-3">
              {module.benefits.map((b, i) => (
                <li key={i} className="flex items-start gap-3 text-sm text-slate-700">
                  <svg className="w-4 h-4 text-indigo-500 mt-0.5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M9 12l2 2 4-4" /></svg>
                  {b}
                </li>
              ))}
            </ul>
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <button 
            ...
            className="flex-1 py-4 bg-[#7338a0] text-white rounded-2xl font-bold hover:bg-[#4a2574] transition-all shadow-lg shadow-indigo-100 flex items-center justify-center gap-2"
          >
            Request Free Demo
          ...
          <button 
            className="flex-1 py-4 border border-slate-200 text-[#0f0529] rounded-2xl font-bold hover:bg-slate-50 transition-all"
            onClick={onClose}
          >
            Close Details
          </button>
